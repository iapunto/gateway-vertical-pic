...
def demo_events():
    ...
    try:
        ...
        events_module = __import__('events.event_manager', fromlist=['subscribe_event', 'emit_event'])
        ...
        emit_event("plc.initialized", {"plc_id": "DEMO-PLC-001", "type": "delta"})
        emit_event("plc.connected", {"plc_id": "DEMO-PLC-001", "ip": "192.168.1.100"})
...
def demo_health_check():
    ...
    try:
        ...
        health_module = __import__('health.health_checker', fromlist=['HealthChecker'])
...
def demo_metrics():
    ...
    try:
        # Importar usando rutas relativas
        monitoring_module = __import__('monitoring.metrics_collector', fromlist=['get_metrics_collector'])
        get_metrics_collector = getattr(monitoring_module, 'get_metrics_collector')
...
def demo_gateway_functionality():
    ...
    try:
        ...
        for plc in plcs:
            print(f"  - {plc.get('id', 'unknown')}: {plc.get('ip', 'unknown')}:{plc.get('port', 3200)}")
