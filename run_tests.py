...
def run_integration_tests():
    ...
    try:
        ...
        test_file = os.path.join(os.path.dirname(__file__), 'src', 'test_plc_integration_standalone.py')
...
def run_plc_tests():
    ...
    try:
        # Verificar si existe el archivo de pruebas
        test_file = os.path.join(os.path.dirname(__file__), 'tests', 'test_plc_communication.py')
        if os.path.exists(test_file):
            import subprocess
            import sys
...
def main():
    """Función principal"""
    parser = argparse.ArgumentParser(description="Ejecutar pruebas del Gateway Local")
    ...
    parser.add_argument("--plc", action="store_true", help="Ejecutar solo pruebas de PLC")
    parser.add_argument("--all", action="store_true", help="Ejecutar todas las pruebas (por defecto)")
