...
def send_custom_command(plc_id: Optional[str] = None, command: int = 0, argument: Optional[int] = None):
    ...
    print(
        f"🧪 Enviando comando personalizado: comando={command}, argumento={argument}")
    ...
    try:
        ...
        # Enviar comando
        print(
            f" Enviando comando {command} con argumento {argument} al PLC {plc_id}...")
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "move":
            if len(sys.argv) >= 3:
                ...
                print(
                    "Uso: python test_plc_commands.py move <posición> [plc_id]")
            ...
            if len(sys.argv) >= 4:
                ...
                argument = int(
                    sys.argv[3]) if sys.argv[3].lower() != "none" else None
                plc_id = sys.argv[4] if len(sys.argv) > 4 else None
                send_custom_command(plc_id, command, argument)
            else:
                print(
                    "Uso: python test_plc_commands.py custom <comando> <argumento|none> [plc_id]")
        else:
            ...
            print(
                "  python test_plc_commands.py custom <comando> <argumento|none> [plc_id]")
            return 1
    else:
